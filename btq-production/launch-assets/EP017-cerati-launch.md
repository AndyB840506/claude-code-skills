# EP.017 — Soda Stereo / Cerati · Paquete de lanzamiento

**Generado:** 2026-06-13 (sáb, día de grabación) · Laswell / episode-launch
**Live:** domingo 14-jun 8:00 PM (hora Colombia)
**Spotify URL:** https://open.spotify.com/episode/0LJ22lLMgfWh3wLbbgNhxC (en vivo, confirmada por Andy 2026-06-22)
**Fuentes:** guion [EP017-cerati-guion.html](EP017-cerati-guion.html) · SRT `E:\Transcriptor\transcripciones\BTQ EP 17.srt` · artwork [EP017-cerati-artwork.md](EP017-cerati-artwork.md)

---

## A · Spotify SEO

**Título (fórmula obligatoria):**
```
EP.17 — Soda Stereo: el liderazgo que sigue sonando cuando ya no estás
```
*(referente "Soda Stereo" + keyword buscable "liderazgo"; numeración EP.17 dos dígitos; sin sufijo "| Behind the Queue")*

**Preview (primeros ~100 chars, antes del "...más"):**
```
En la cima, Cerati cambió lo que ya funcionaba. La lección de liderazgo que tu operación necesita oír.
```

**Descripción completa (~320 palabras):**

> ⚠️ **Spotify acepta HTML.** Para evitar palabras pegadas (Spotify colapsa los saltos
> manuales a mitad de párrafo: "la crisis" → "lacrisis"), pegar la **versión HTML de
> abajo** (cada párrafo en `<p>`). La versión texto plano queda como referencia; si se
> usa, mantener cada párrafo en UNA línea separada por línea en blanco — no re-wrappear.

**Texto plano (referencia / fallback):**
```
Gustavo Cerati llevó a Soda Stereo a reinventarse en el punto más alto de su carrera —con Canción Animal vendiendo medio millón de copias— hacia un disco que nadie le pidió. Treinta años después, esa decisión es una de las lecciones de liderazgo más completas que vas a encontrar fuera de un libro de management.

Este episodio usa la historia de Cerati como espejo para una pregunta incómoda de liderazgo que todo supervisor o gerente de un call center o BPO termina enfrentando: ¿estás cambiando lo que todavía funciona antes de que se rompa, o esperas a que la crisis decida por ti? Y la más dura de todas: ¿qué de lo que construiste en tu operación seguiría sonando si mañana ya no estuvieras ahí?

Vas a salir con tres herramientas concretas para esta semana: la pregunta de los viernes para detectar lo que funciona por inercia y no por criterio; cómo escribir el final de un proceso antes de necesitarlo, como Soda Stereo planeó su propia despedida; y por qué agradecer a tu gente en el momento alto —no en la entrevista de salida— es lo que separa un buen desempeño de un legado real.

Por el camino: el giro de Dynamo, la noche de "Gracias Totales" en River Plate, el regreso de 2007 que convocó a más de un millón de personas, y el dato que lo cierra todo —la gira de 2020 que arrancó en El Campín de Bogotá, sin Cerati, y aun así llenó estadios. Con guiños a Steve Jobs, Jim Collins y David Bowie.

Y dime, sin adornos: si mañana faltaras tres meses sin aviso, ¿qué de lo que construiste sigue funcionando sin ti… y qué se apaga? Te leo en los comentarios.

Yo soy Andy. Y como diría el gran Gustavo Cerati: "Gracias… totales".

Escúchalo ahora en Spotify.

📩 andy@behind-thequeue.com | 🌐 behind-thequeue.com
📸 Instagram: @behindthequeue84 | 🎵 TikTok: @behind.the.queue | 📘 Facebook: facebook.com/behindthequeue
💼 LinkedIn: linkedin.com/company/behind-the-queue
```

**HTML — pegar esto en Spotify (campo descripción acepta HTML):**
```html
<p>Gustavo Cerati llevó a Soda Stereo a reinventarse en el punto más alto de su carrera —con Canción Animal vendiendo medio millón de copias— hacia un disco que nadie le pidió. Treinta años después, esa decisión es una de las lecciones de liderazgo más completas que vas a encontrar fuera de un libro de management.</p>
<p>Este episodio usa la historia de Cerati como espejo para una pregunta incómoda de liderazgo que todo supervisor o gerente de un call center o BPO termina enfrentando: ¿estás cambiando lo que todavía funciona antes de que se rompa, o esperas a que la crisis decida por ti? Y la más dura de todas: ¿qué de lo que construiste en tu operación seguiría sonando si mañana ya no estuvieras ahí?</p>
<p>Vas a salir con tres herramientas concretas para esta semana: la pregunta de los viernes para detectar lo que funciona por inercia y no por criterio; cómo escribir el final de un proceso antes de necesitarlo, como Soda Stereo planeó su propia despedida; y por qué agradecer a tu gente en el momento alto —no en la entrevista de salida— es lo que separa un buen desempeño de un legado real.</p>
<p>Por el camino: el giro de Dynamo, la noche de "Gracias Totales" en River Plate, el regreso de 2007 que convocó a más de un millón de personas, y el dato que lo cierra todo —la gira de 2020 que arrancó en El Campín de Bogotá, sin Cerati, y aun así llenó estadios. Con guiños a Steve Jobs, Jim Collins y David Bowie.</p>
<p>Y dime, sin adornos: si mañana faltaras tres meses sin aviso, ¿qué de lo que construiste sigue funcionando sin ti… y qué se apaga? Te leo en los comentarios.</p>
<p>Yo soy Andy. Y como diría el gran Gustavo Cerati: "Gracias… totales".</p>
<p>Escúchalo ahora en Spotify.</p>
<p>📩 <a href="mailto:andy@behind-thequeue.com">andy@behind-thequeue.com</a> | 🌐 <a href="https://behind-thequeue.com">behind-thequeue.com</a><br>📸 Instagram: <a href="https://instagram.com/behindthequeue84">@behindthequeue84</a> | 🎵 TikTok: <a href="https://tiktok.com/@behind.the.queue">@behind.the.queue</a> | 📘 Facebook: <a href="https://facebook.com/behindthequeue">facebook.com/behindthequeue</a><br>💼 LinkedIn: <a href="https://linkedin.com/company/behind-the-queue">linkedin.com/company/behind-the-queue</a></p>
```
*(Si el editor de Spotify limpia los `<a>`, el texto del link queda visible igual — sin problema.)*

**Tags / keywords Spotify (comma-separated):**
```
liderazgo, gestión del cambio, BPO, call center, contact center, servicio al cliente,
operaciones, Soda Stereo, Gustavo Cerati, rock en español, legado, liderazgo de equipos,
Behind the Queue, podcast en español, LATAM, Colombia
```

---

## B · Plan Social — calendario 4 días

> **Fechas verificadas:** hoy es sábado 13-jun; lanzamiento domingo 14-jun 8 PM.
> La ventana de intriga (jue 11 / vie 12) ya está corriendo o cerrada (los teasers
> se estaban generando en Flow). Si aún no salió la intriga, publicar HOY la pista
> como "última señal antes del estreno". Secuencia: intriga → calentamiento →
> lanzamiento → pico lunes → refuerzo martes.

| Día | Plataforma | Copy |
|-----|-----------|------|
| **Vie 12 (intriga)** | LinkedIn | *(prioritaria)* "Hay un momento en toda operación que es el más peligroso de todos —y casi nadie lo ve venir. No es la crisis. No es el número en rojo. Es justo lo contrario: es eso que 'funciona bien' y que por eso mismo nadie se atreve a tocar. El domingo te cuento qué tiene que ver esto con uno de los líderes más grandes que dio la música en español. 🎙️ Nuevo episodio, domingo 8 PM. #Liderazgo #BPO #ContactCenter #BehindTheQueue #Operaciones" |
| Vie 12 (intriga) | Instagram / Facebook | "¿Cambiarías algo en tu operación que TODAVÍA está funcionando? 🤔 Domingo 8 PM se viene un episodio sobre el líder que se atrevió a hacerlo en su mejor momento. Una pista: definió el sonido de una generación. #BehindTheQueue #PodcastEnEspañol #Liderazgo #BPO #ContactCenter #ServicioAlCliente #Operaciones #LATAM #Colombia #CustomerExperience #NuevoEpisodio" |
| Vie 12 (intriga) | Story slides (3) | 1) Micrófono solo bajo el spotlight + "LO QUE CONSTRUISTE…" · 2) "¿SIGUE SONANDO SIN TI?" · 3) sticker countdown "DOMINGO 8 PM" |
| Vie 12 (intriga) | TikTok | "El líder que cambió lo que ya funcionaba 🎤 Domingo, nuevo episodio. #liderazgo #bpo #callcenter" |
| **Sáb 13 (calentamiento)** | Instagram / Facebook | *(orgánico, BTS)* "Día de grabación en la Guarida… digo, en el estudio 🎙️ Hoy cae uno de los episodios que más he disfrutado escribir. Mañana 8 PM lo sabrán por qué. #BehindTheQueue #DetrásDeCámaras #PodcastEnEspañol #Liderazgo #BPO #Podcast #Colombia" |
| Sáb 13 (calentamiento) | Story | Foto/clip real del setup de grabación + sticker countdown a domingo 8 PM |
| Sáb 13 | TikTok | "Grabando el episodio de mañana 👀 pista: rock en español + liderazgo. #podcast #bts #liderazgo" |
| **Dom 14 · 8:00 PM (lanzamiento)** | LinkedIn | *(prioritaria — link de Spotify en PRIMER COMENTARIO)* "En 1990 Soda Stereo tenía la fórmula perfecta: su disco más vendido, el público rendido. Y justo ahí, Cerati la rompió. Grabó Dynamo —el disco que nadie le pidió y que partió en dos a su público. Treinta años después, la crítica lo señala como el momento en que dejaron de ser 'una banda exitosa' para ser 'una banda que importaba'. Hay una versión de esto en tu piso de operaciones: ese guion, ese flujo, esa métrica que 'convierte bien' y que por eso nadie cuestiona. El líder que espera a que algo se rompa para cambiarlo, ya llegó tarde. Hoy estrenamos un episodio sobre cambiar a tiempo, cerrar bien y construir algo que siga sonando cuando tú ya no estés. 👇 Link en el primer comentario. Y dime: ¿qué de lo que construiste seguiría funcionando si faltaras tres meses sin aviso? #Liderazgo #BPO #ContactCenter #BehindTheQueue #Operaciones" |
| Dom 14 · 8 PM | Primer comentario (LinkedIn) | "🎧 Episodio completo aquí: https://open.spotify.com/episode/0LJ22lLMgfWh3wLbbgNhxC" |
| Dom 14 · 8 PM (lanzamiento) | Instagram / Facebook | "🎙️ NUEVO EPISODIO — Soda Stereo: el liderazgo que sigue sonando cuando ya no estás. La historia de Cerati como espejo de una pregunta que todo líder de operaciones evita: ¿qué dejas construido para cuando ya no estés? Link en bio 🔗 Y cuéntame: ¿qué de lo que construiste sigue sonando sin ti? #BehindTheQueue #PodcastEnEspañol #NuevoEpisodio #SodaStereo #Cerati #Liderazgo #BPO #ContactCenter #ServicioAlCliente #Operaciones #LATAM #Colombia #CustomerExperience" |
| Dom 14 · 8 PM | Story (3) | 1) portada del episodio · 2) sticker link directo a Spotify · 3) pregunta abierta "¿qué sigue sonando sin ti?" |
| Dom 14 · 8 PM | TikTok | "Soda Stereo hizo una gira SIN Cerati… y llenó estadios. Por qué eso es liderazgo 🎤 Nuevo episodio. #liderazgo #sodastereo #bpo" |
| **Lun 15 · 7–8 AM (pico de escucha)** | LinkedIn | *(arrancando la semana — link en primer comentario)* "Arrancando la semana con una sola pregunta para tu piso de operaciones: ese proceso que llevas tres años usando porque 'convierte bien'… ¿sigue ahí porque es lo correcto, o porque nadie se ha detenido a cuestionarlo? En el episodio de ayer la llamo 'la pregunta de los viernes'. Pruébala esta semana —antes de que una crisis te obligue a revisarlo con afán. 🎧 Episodio en el primer comentario. #Liderazgo #BPO #ContactCenter #BehindTheQueue #GestiónDelCambio" |
| Lun 15 | Primer comentario | "🎧 'Soda Stereo: el liderazgo que sigue sonando' — https://open.spotify.com/episode/0LJ22lLMgfWh3wLbbgNhxC" |
| **Mar 16 (refuerzo)** | LinkedIn | *(quote card Q1 — link en primer comentario)* "'El legado no se mide en cuánto tiempo estuviste presente. Se mide en cuánto tiempo sigue funcionando, sin ti, lo que dejaste construido.' Soda Stereo salió de gira en 2020 sin Cerati —arrancó en El Campín, Bogotá, ante 30.000 personas— y llenó estadios. Lo que él construyó no era su presencia: era algo que cualquier voz podía encender. ¿Tu operación está construida así? 🎧 Episodio completo en el primer comentario. #Liderazgo #Legado #BehindTheQueue #BPO #ContactCenter" |
| Mar 16 (refuerzo) | Instagram / Facebook | *(quote card Q1 o Q4)* "El éxito necesita que estés presente. El legado sigue funcionando cuando ya te fuiste. 🎙️ EP.17 disponible — link en bio. ¿Qué estás construyendo: éxito o legado? #BehindTheQueue #PodcastEnEspañol #Liderazgo #Legado #SodaStereo #Cerati #BPO #ContactCenter #ServicioAlCliente #Operaciones #LATAM #Colombia #CustomerExperience #NuevoEpisodio" |
| Mar 16 | TikTok | "'Era algo que cualquier voz podía encender.' La gira de Soda Stereo sin Cerati explica qué es el legado. #liderazgo #legado #podcast" |

**Notas de plataforma:**
- **LinkedIn = prioritaria** (audiencia núcleo hombre 35–44 supervisor/gerente BPO; 15% escucha en desktop en el trabajo). 5–8 hashtags, link de Spotify SIEMPRE en primer comentario.
- **Instagram / Facebook:** 10–15 hashtags, link en bio, día de lanzamiento cierra con pregunta.
- **La pregunta interpela al OYENTE, no al personaje** (efecto EP.016: 7 comentarios). Aquí: "¿qué sigue sonando sin ti?" en las 4 plataformas.
- **Quotes/clips para promo del catálogo:** usar EP.012 Bohemian Rhapsody como puerta de entrada (el que el algoritmo ya empuja), no el más reciente.

---

## C · YouTube

**Título (hook largo, con keyword):**
```
La gira que Soda Stereo hizo SIN Cerati — y la lección de liderazgo que tu operación necesita | EP.17 | Behind the Queue
```

**Descripción (5 bloques):**
```
¿Cuándo fue la última vez que cambiaste algo en tu operación que TODAVÍA estaba
funcionando —no porque alguien te lo exigiera, sino porque viste, antes que nadie,
que ese "funcionando bien" tenía fecha de vencimiento? Esa es la pregunta honesta
con la que arranca este episodio.

Usamos la historia de Gustavo Cerati y Soda Stereo como espejo para una de las
lecciones de liderazgo más completas que vas a encontrar fuera de un libro de
management: cuándo cambiar lo que funciona, cómo cerrar un ciclo en su mejor momento,
y qué dejar construido para que siga sonando cuando tú ya no estés. Si lideras un
equipo en un call center, un BPO o cualquier operación, este episodio es para ti.

CONTENIDO DEL EPISODIO
0:00 Intro
2:18 La pregunta honesta
5:27 Quién fue Cerati y Soda Stereo
10:13 El Puente: de Dynamo a tu operación
13:02 Los datos: Canción Animal y Me Verás Volver
15:26 Las lecciones: Persiana Americana, River Plate, el regreso de 2007
20:07 Referencias cruzadas: Steve Jobs, Jim Collins y David Bowie
24:27 Aplicable hoy: 3 herramientas para esta semana
27:56 Mito o realidad (y la gira de 2020 sin Cerati)
32:11 Recomendaciones de Andy
35:48 La gerente que volvió de licencia
38:14 Lo que viene + cierre
40:07 Outro

ENCUENTRA BTQ EN
🌐 behind-thequeue.com
🎧 Spotify: https://open.spotify.com/episode/0LJ22lLMgfWh3wLbbgNhxC
📩 andy@behind-thequeue.com
💼 LinkedIn: Andrés Bermúdez Rodríguez · linkedin.com/company/behind-the-queue
📸 Instagram: @behindthequeue84

#BehindTheQueue #Liderazgo #SodaStereo #Cerati #BPO #ContactCenter #PodcastEnEspañol #Operaciones
```

**Tags (campo Tags de YouTube Studio, comma-separated, 15–20):**
```
liderazgo, soda stereo, gustavo cerati, rock en español, gestión del cambio, BPO,
call center, contact center, servicio al cliente, operaciones, liderazgo de equipos,
legado, podcast en español, podcast de liderazgo, behind the queue, LATAM, Colombia,
desarrollo de equipos, customer experience
```

**Thumbnail (16:9):** texto "LA GIRA / SIN CERATI" (ya en el artwork; 3–5 palabras, alto contraste, Bebas Neue uppercase). Banda a la derecha, texto a la izquierda, sin footer.

---

## D · Artwork / Flow — orden de generación

Todos los prompts viven validados en [EP017-cerati-artwork.md](EP017-cerati-artwork.md).
Orden sugerido para pegar en Flow, por fecha de uso:

| # | Asset | Formato | Para | Estado |
|---|-------|---------|------|--------|
| 1 | **Portada** | 1:1 | Spotify cover | ✅ generada y aprobada (12-jun) |
| 2 | Teaser intriga (micrófono solo) | 9:16 | Vie/sáb intriga | prompt listo |
| 3 | Teaser countdown (batería) | 9:16 | Sáb BTS | prompt listo |
| 4 | Lanzamiento (deriva de portada) | 9:16 | Dom 8 PM story | editar sobre portada |
| 5 | Card "gira sin Cerati" | 1:1 | Lun LinkedIn | prompt listo |
| 6 | YouTube thumbnail "LA GIRA / SIN CERATI" | 16:9 | al subir video | prompt listo |
| 7 | Teaser "Gracias Totales" | 9:16 | Mar refuerzo | prompt listo |
| 8 | **Q1 "El legado"** | 16:9 | Mar LinkedIn refuerzo | ✅ validado vs audio |
| 9 | **Q2 "¿Qué se apaga?"** | 16:9 | Mar / 2º post lun | ✅ validado vs audio |
| 10 | **Q3 "Cualquier voz"** | 16:9 | refuerzo gira 2020 | ✅ validado vs audio (texto "una presencia") |
| 11 | **Q4 "Éxito vs legado"** | 16:9 | refuerzo / IG mar | ✅ nuevo, validado vs audio |

**Recordatorios Flow (de brand-constants):** conteo de figuras exacto · texto letra por
letra con zoom · cero caras · sin tildes en el texto de imagen (la frase con tildes va
en el caption) · si Flow genera la escena pero omite el texto, NO regenerar — modo
edición "On this same image, keep everything identical and add text…".

---

## Next steps

1. **Generar imágenes en Flow** según la tabla D (los Q1–Q4 ya están habilitados por la validación de hoy).
2. **Publicar en Spotify** dom 14-jun 8 PM → pegar aquí la URL en vivo (verificada en el browser) y propagarla a los CTAs de §B/§C.
3. Tras publicar: registro SafeCreative + rotación de la grilla de la web ([post-publish.md](../../.claude/skills/episode-launch/workflows/post-publish.md)).
4. Roadmap: al confirmar Spotify, EP.017 → `en Spotify`.

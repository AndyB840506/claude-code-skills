...
3. **Extract structured signals** — output `companySize`, `industry`, `techStack`, `keyPersonnel`, `painSignals` as structured JSON
4. **Store** results wherever the user tracks accounts (sheet, CRM, etc.)

### Output fields
WCC: `text` (per page), `url`
LinkedIn: `employeeCount`, `industry`, `headquarters`, `description`, `specialties`
Extracted: `companySize`, `industry`, `techStack`, `keyPersonnel`, `painSignals`
...
2. **Filter by category + upvote threshold** on extracted `upvotes`, `category`
...
6. **Output** to wherever the pipeline is tracked + alert on top matches
...
### Pipeline
1. **Trigger on calendar event** (e.g. event starting in 30 min)
...
5. **Synthesize a brief** — key topics, recent signals, suggested talking points
6. **Deliver** to the user ahead of the meeting
...
### Gotcha
LinkedIn profile URLs must come from the calendar event description or a linked CRM record — they won't auto-resolve from email addresses. Without a valid `profileUrl`, the HarvestAPI Actors return empty results.

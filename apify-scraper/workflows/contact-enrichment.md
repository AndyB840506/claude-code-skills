...
3. **Dedup and verify** — dedup by email domain, optionally verify with a Hunter.io/ZeroBounce API call
...
3. **Filter by ICP criteria** — filter on `headline` or `companyName`
4. **Draft outreach** — generate a personalized message using `commentText` + `headline`
...
### Pipeline
1. **Receive form data** (webhook from HubSpot/Typeform/custom form)
2. **Extract company domain** from the submitted email
...
5. **Extract signals** — pull `companySize`, `industry`, `techStack`, `ICPFit` score from crawl text
6. **Route** — high-ICP leads to a priority channel, others to a nurture sequence

...
**Important:** Cost estimates in the workflow playbooks are approximate and may be inaccurate. Always present them as rough guidance with the disclaimer above, never as exact amounts.
...
Actor input is one JSON object, not an array. `--input` accepts inline JSON object input only; wrap inline JSON in quotes to avoid shell parsing issues. For JSON files or complex inputs, use `--input-file input.json`. If the CLI reports parse, path, or object-shape input errors, inspect the schema again with `apify actors info "ACTOR_ID" --input --json`.
    ...
    apify actors call "ACTOR_ID" --input '{"maxItems":10}' --json

Prefer this for larger inputs:

    apify actors call "ACTOR_ID" --input-file input.json --json

**Windows/PowerShell specifically:** prefer `-f <file>` over inline `-i '<json>'` by
default, not just for "large" inputs. PowerShell's argument passing to the
underlying Node executable (through the `apify.ps1` wrapper) can mangle inline JSON
quoting even when the JSON is correctly single-quoted — this produces a confusing
`Cannot parse JSON input` error with no indication the quoting was the cause.
Write the input to a temp `.json` file and pass it with `-f` instead.

**`-o` and `--json` are mutually exclusive.** `apify actors call ... -o --json`
fails immediately with "You cannot use both the --json and --output-dataset
flags." Pick one: `-o` prints the dataset as readable output, `--json` formats
the whole CLI response as JSON.
...
Raw HTTP requests or self-hosted Puppeteer hit common walls that Apify Actors handle transparently:

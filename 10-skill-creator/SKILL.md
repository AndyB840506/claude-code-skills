---
name: 10-skill-creator
description: "Crea nuevas skills de Claude Code desde cero. Es la skill que hace skills. Usa esta skill cuando el usuario quiera crear su propia skill, automatizar un flujo de trabajo, hacer que Claude repita un proceso, crear un comando personalizado, o convertir algo que hace manualmente en algo automático. Triggers ES: 'crea una skill', 'quiero hacer una skill', 'skill personalizada', 'automatizar esto como skill', 'crear un comando para Claude Code', 'convierte esto en una skill', 'quiero que Claude siempre haga X', 'skill para Y'. English triggers: create a skill, new skill, make a skill, automate this as a skill, build a custom command, I want Claude to always do X, skill for Y, turn this into a skill, make a workflow, create a command for Claude."
---
...
### 13 Principios de diseño de skills (aprendidos de nuestras 9 skills anteriores)

Estos principios son los que hacen que una skill sea realmente buena:

**Core principles (1-10)** — Esenciales para cualquier skill.  
**Advanced principles (11-13)** — Para skills conversacionales y con agentes.
...
- **Brand Kit:** Si el proyecto tiene un Brand Kit configurado, pasarlo en cada llamada a `generate-design`, `generate-design-structured` o `request-outline-review`. El `brand_kit_id` es específico del proyecto — obtenerlo del contexto del usuario, no hardcodearlo en la skill.

### Patrones de hooks (ciclo de vida de sesión)

Claude Code soporta hooks que se disparan automáticamente en eventos del ciclo de vida. Úsalos cuando la skill necesite ejecutarse sin que el usuario la invoque manualmente.

| Hook | Cuándo se dispara | Cuándo usarlo en una skill |
|---|---|---|
| `PreCompact` | Justo antes de `/compact` | Guardar estado o contexto crítico antes de que se comprima |
| `SessionEnd` | Al cerrar la sesión | Auto-disparar `session-close` o `handoff` sin que el usuario lo escriba |
| `PostToolUse` (if) | Después de cualquier tool call | Flow control condicional (ej: "solo dispara handoff si hay cambios en git") |

**Cuándo mencionar hooks en tu skill:**
- Si la skill se diseña para ejecutarse al final de cada sesión → menciona `SessionEnd` como opción de auto-trigger
- Si la skill necesita preservar datos antes de compactación → menciona `PreCompact`
- Si la skill necesita lógica condicional basada en eventos del sistema → `PostToolUse` con condición `if`

**Ejemplo de configuración en `settings.json`:**
```json
{
  "hooks": {
    "SessionEnd": [{ "hooks": [{ "type": "command", "command": "echo 'Fin de sesión — ejecutando /session-close'" }] }],
    "PreCompact": [{ "hooks": [{ "type": "command", "command": "echo 'Compactando — guarda contexto importante'" }] }]
  }
}
```
...
- **`/goal`** — Declara una condición de finalización para workflows multi-paso. Úsalo en skills que iteran hasta cumplir un criterio (ej: "continúa hasta que todos los tests pasen").
- **`/loop`** — Ejecuta un prompt en intervalos recurrentes. Útil para skills de monitoreo (ej: "revisa el build cada 5 minutos").
- **`/schedule`** — Crea agentes remotos con schedule cron. Para skills que deben ejecutarse en horario fijo sin intervención del usuario.

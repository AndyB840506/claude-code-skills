...
## Implementación — Snippets requeridos (incluir siempre)

Claude genera el HTML completo inline. Estos bloques deben estar presentes en TODO output:

**CSS variables base (dentro de `<style>`):**
```css
:root {
  --bg: #0f0f0f; --card: #1a1a1a; --text: #e8e8e8;
  --accent: [color_principal del perfil]; --accent2: [color_secundario];
...
.chip-pausa { background: #f5c518; color: #000; border-radius: 4px; padding: 2px 8px; font-size: .8em; }
.chip-sfx   { background: #4a9eff; color: #fff; border-radius: 4px; padding: 2px 8px; font-size: .8em; }
.nota-box   { border-left: 3px solid #f5c518; background: #2a2a2a; padding: 1rem; }
@media print { .nota-box { display: none; } nav { display: none; } }
```

**JS para botón Copiar (inline, sin CDN):**
```html
<button onclick="navigator.clipboard.writeText(this.previousElementSibling.value).then(()=>{this.textContent='✓ Copiado';setTimeout(()=>this.textContent='Copiar',2000)})">Copiar</button>
```

**Tabs CSS puro (sin JS) — para días del social media:**
```html
<input type="radio" name="tabs" id="d1" checked hidden>
<input type="radio" name="tabs" id="d2" hidden>
<label for="d1">Día 1</label>
<label for="d2">Día 2</label>
<div class="tab-content" id="content-d1">...</div>
<div class="tab-content" id="content-d2">...</div>
<!-- CSS: input:checked + label + .tab-content visibility toggling -->
```

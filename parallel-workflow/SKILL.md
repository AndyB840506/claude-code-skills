...
Splits any large task into independent subtasks and runs them simultaneously using parallel subagents. Results are collected and synthesized into one final output.
...
Pass `run_in_background: true` to each Agent call — without this, agents run sequentially instead of in parallel.
Each agent receives:

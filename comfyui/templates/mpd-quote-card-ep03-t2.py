# Quote cards de MPD EP.03 (T2*E3) "La Bestia que el rock volvio inmortal" - 1920x1080,
# sistema La Guarida. Copiado de mpd-quote-card-ep02-t2.py y adaptado: escena/paleta/
# rutas vienen de mpd-portada-ep03-t2.py. No reusar tal cual para el proximo expediente.
#
# Uso: python mpd-quote-card-ep03-t2.py
...
_ep = _load("mpd_portada_ep03_t2", "mpd-portada-ep03-t2.py")
...
# Verbatim del SRT real (E:\Transcriptor\transcripciones\MPD EP 03.srt), no del guion.
# Una por acto + un cierre humanizador, mismo patron de EP.006/EP.02 (4 cards, y_center
# distinto para no repetir el mismo encuadre de la escena).
QUOTES = [
    ("Q1", "07:30", 0.30, "No se lo inventó él, se lo puso su madre."),
    ("Q2", "13:45", 0.38, "Casi 70 años después, ese hueco sigue sin llenarse."),
    ("Q3", "20:43", 0.45, "Bowie nunca lo explicó del todo. Décadas después, ya sobrio, "
                          "hablaba de esos años de Los Ángeles casi sin reconocerse, como "
                          "si estuviera contando la historia de otra persona."),
    ("Q4", "27:48", 0.52, "La fama no siempre necesita que lo quieran a uno. A veces solo "
                          "necesita que no lo puedan olvidar."),
]
...
def compose(tag, timestamp, y_center, text):
    ...
    path = os.path.join(OUT_DIR, "MPD-T2E03-%s-1920x1080.png" % tag)
    im.save(path)

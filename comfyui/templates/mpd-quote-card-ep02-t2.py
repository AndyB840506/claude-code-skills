# Quote cards de MPD EP.02 (T2*E2) "El rock y el diablo" - 1920x1080, sistema La Guarida.
#
# Copiado de mpd-quote-card-t2.py (EP.006/T2E01) y adaptado: escena/paleta/rutas vienen
# de mpd-portada-ep02-t2.py, no del compositor de EP.006 (cada episodio tiene su propia
# escena hardcodeada, mismo patron documentado ahi). No reusar tal cual para el proximo
# expediente -- copiar y adaptar de nuevo.
#
# Uso: python mpd-quote-card-ep02-t2.py
...
_ep = _load("mpd_portada_ep02_t2", "mpd-portada-ep02-t2.py")
...
# Verbatim del SRT real (E:\Transcriptor\transcripciones\MPD EP 02.srt), no del guion.
# Una por acto + un cierre humanizador, mismo patron de EP.006 (4 cards, y_center
# distinto para no repetir el mismo encuadre de la escena).
QUOTES = [
    ("Q1", "05:48", 0.42, "Nadie sabe qué le dijo el hombre del cruce, si le dijo algo. "
                          "Nadie sabe si hubo un trato o solo un gesto."),
    ("Q2", "17:52", 0.52, "Crowley no inventó algo raro de ese lugar, lo heredó y lo alimentó."),
    ("Q3", "29:45", 0.62, "Convierte \u2018yo no oigo nada\u2019 en la prueba de que sí está "
                          "funcionando. Si usted no lo escucha, es justamente porque va "
                          "dirigido a la parte de usted que no controla."),
    ("Q4", "35:44", 0.72, "Yo no soy capaz de juzgar a esos padres. Nadie que no haya "
                          "perdido un hijo puede buscar una explicación afuera."),
]
...
def compose(tag, timestamp, y_center, text):
    ...
    path = os.path.join(OUT_DIR, "MPD-T2E02-%s-1920x1080.png" % tag)
    im.save(path)

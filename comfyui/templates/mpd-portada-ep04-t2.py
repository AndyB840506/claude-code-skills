# Portada de MPD EP.04 (T2*E4) "Paul is Dead" - copiado de mpd-portada-ep03-t2.py y
# adaptado (misma decision de Andres: sin draw_numeral, la escena ya integra el
# concepto sin necesitar un numero central).
#
# Escena: estudio "La Guarida" con marco ovalado (espejo antiguo empanado, sin figura
# ni cara reflejada) sobre la chimenea, vinilos + periodico/whisky/puro en mesa
# lateral. Generada con Z-Image Turbo (seed 4202026) a 1024x1024, upscaleada a
# 3000x3000 con RealESRGAN, pasada por night_grade variante E.
#
# Uso: python mpd-portada-ep04-t2.py
...
TITLE = "Paul is Dead"
TAGLINE = "El chiste que medio mundo decidió creerse."
...
def build_1x1(scene):
    ...
    return save(im, "MPD-T2E04-PORTADA-3000.jpg")


def build_16x9(scene, y_center=0.30):
    im = crop_aspect(scene, 16 / 9, y_center=y_center).resize((1920, 1080), Image.LANCZOS)
    ...
    return save(im, "MPD-T2E04-16x9-FINAL.png")


def build_9x16(scene, y_center=0.46):
    im = crop_aspect(scene, 1080 / 1920, y_center=y_center).resize((1080, 1920), Image.LANCZOS)
    im = scrim_bands(im, top_frac=0.16, top_alpha=140, **BOTTOM_SCRIM)
    ...
    return save(im, "MPD-T2E04-9x16-FINAL.png")


if __name__ == "__main__":
    scene = Image.open(SCENE).convert("RGB")
    print("escena:", SCENE, "| ya trae night_grade variante E aplicado")
    # Verificar que el titulo quepa en el piso de fuente del formato mas angosto (9:16)
    # antes de confiar en el render (gotcha documentado, EP.03 copiado de EP.02).
    test_font = ImageFont.truetype(F_DISPLAY, int(1920 * 0.018))
    fits = test_font.getlength(TITLE) <= (1080 - 2 * int(1080 * 0.08))
    print("titulo cabe en piso 9:16:", fits)
    build_1x1(scene)
    build_16x9(scene)
    build_9x16(scene)

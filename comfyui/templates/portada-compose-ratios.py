# ############################################################################
# Derivado de portada-compose.py (muerto) para la EXCEPCION de EP.024.
...
# --- wordmark ---
# FIX 2026-08-01: el tamano del wordmark se calculaba solo con la ALTURA, y en
# 9:16 THE QUEUE se salia del cuadro. Ahora se achica hasta caber en el ANCHO.
_s = int(H * 0.108)
while _s > 8:
    fw = ImageFont.truetype(DISP, _s)
    if max(fw.getlength(_l) for _l in WORD) <= W - 2 * M:
        break
    _s -= 2
y = int(H * 0.058)
ink_bottom = y

...
⚠️ **Important for Windows PowerShell environments:**

When committing with multi-line messages in PowerShell, use standard `git commit -m` syntax, NOT heredoc (`@'...'@`):

**❌ WRONG (causes EOF errors in PowerShell):**
...
```powershell
git commit -m "Multi-line message using standard quotes"
```

PowerShell heredoc syntax differs from Bash. Always use the simpler `-m "message"` format for maximum compatibility.

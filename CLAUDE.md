...
- **Un `\n` dentro de un heredoc de python lanzado por la herramienta Bash llega convertido en salto de línea REAL.** Así que un patrón de búsqueda que contenga `\n` literal —buscar dentro de un string de PHP/JS/Go, que es donde vive como dos caracteres— nunca matchea, y el script reporta "0 ocurrencias" sobre texto que sí está ahí. Mordió 3 veces el 2026-08-26. **Construir el backslash con `chr(92)`** (`NL = chr(92) + 'n'`) en vez de escribirlo en el literal. Corolario que salvó las tres veces: un script de reemplazo masivo que **cuenta cada patrón antes de escribir y aborta si alguno da != 1** convierte esto en un aviso en vez de un archivo a medias.
  **Más simple, y es lo que funcionó cada vez:** si el script lleva CUALQUIER barra invertida (regex, `\n` dentro de un string de PHP/JS, rutas), **escribirlo a disco con la herramienta Write y correrlo**, nunca por heredoc **ni por `python -c "…"` en línea** (mismo problema: el shell reinterpreta las barras dentro de las comillas). El 2026-09-18 el heredoc se comió barras 4 veces en una sesión aunque esta regla ya existía: un `\ndash` de un doc quedó como salto de línea real, un `\s` de un test JS quedó como `s`. El 2026-09-22 volvió a pasar con un `python -c` que llevaba un regex: el script reventó al compilar el patrón.
- **Un `git checkout <archivo>` puede devolver el archivo con CRLF aunque estuviera en LF** (autocrlf): un script de reemplazo con patrones en `\n` deja de encontrar nada. Mordió el 2026-09-18 en HireSignal; el script abortó sin escribir (correcto). Patrón que funcionó: al leer, normalizar a LF y recordar el fin de línea original; al escribir, restaurarlo.
- **Python en Windows no entiende las rutas `/c/...` de Git Bash.** La herramienta Bash es Git Bash, pero `python` es el interprete nativo de Windows: pasarle `/c/Users/...` da `FileNotFoundError` aunque el archivo exista y aunque `ls` sobre esa misma ruta funcione en la linea de al lado. Mordio dos veces el 2026-09-07. **Usar `C:/Users/...`** (barras normales, con letra de unidad) en cualquier ruta que vaya dentro de un script de python, aunque el resto del comando use rutas estilo Git Bash.
- **Un `.ps1` se escribe en ASCII puro — un em dash dentro de un string lo parte en silencio.** PS 5.1 lee un `.ps1` UTF-8 **sin BOM** como cp1252, así que `—` (E2 80 94) decodifica a `â€"`, y esa comilla curva final (U+201D) **PowerShell la acepta como delimitador de string**: cierra la cadena a mitad de línea y el `}` siguiente revienta el parseo. El error apunta a una línea que no tiene nada malo (reportó L64 cuando el daño estaba en L62). En comentarios es inofensivo, así que un archivo puede llevar años con em dashes y romperse recién cuando alguien pone uno dentro de comillas. Mordió el 2026-09-22: `install.ps1` llevaba **roto desde julio** sin que se notara, porque `sync.ps1` usa `--` y se corre siempre mientras que `install.ps1` casi nunca. **Verificación binaria** (no leer el archivo a ojo): `[System.Management.Automation.Language.Parser]::ParseFile($p,[ref]$t,[ref]$e)` y exigir `$e.Count -eq 0`, más un conteo de caracteres `>127` en python. Aplica a tildes y ñ igual que al em dash: si un `.ps1` necesita texto acentuado, va en un archivo de datos aparte, no en el script.
...
0. **Baja la continuidad:** ejecuta `C:\Users\andre\repos\claude-continuity\start.ps1`. Hace el pull de continuity Y del kit (cubre el paso 1), restaura la memoria que la otra máquina guardó, y avisa si algo local es más nuevo o si la memoria quedó repartida en varios slugs. **Es la mitad que faltaba:** `session-close` (paso 5) ya subía memoria y config a git, pero nada la bajaba — el arranque solo pulleaba el repo de skills, así que la memoria de la otra máquina se quedaba en `claude-continuity` sin que nadie la trajera. Eso obligaba a escribir «corre install.ps1» a mano dentro de los handoffs (2026-09-22). Si el script falla o no está, cae al paso 1 y dilo.
   - **Nunca pisa trabajo local:** compara contra la fecha de **commit** del archivo, no su mtime — git no preserva mtimes y un pull dejaría todo con fecha de ahora, ganándole a trabajo local genuinamente más nuevo.
   - **Si avisa de varios slugs:** la memoria se guarda por RUTA del workspace, así que abrir el kit desde rutas distintas en cada máquina crea dos memorias que nunca convergen (pasó entre `~/.claude/skills` y `repos/kit-skill-creator`, fusionadas a mano el 2026-09-22). Abrir SIEMPRE `C:\Users\andre\.claude\skills`, en las dos máquinas.
1. **Sincroniza con GitHub:** lo hace el paso 0; solo si ese falló, ejecuta `git pull origin main` Y TAMBIÉN `git -C "$env:USERPROFILE\.claude\skills" pull origin main` — son 2 clones del mismo repo y el global se desactualiza solo (mordió 2026-07-08: 1 mes stale en el portátil, una skill "no existía")
...
**Todo comando que liste variables de entorno imprime solo los nombres con forma de identificador** — `printenv`, `env`, `Get-ChildItem Env:`, `HKCU:\Environment`, no solo al diagnosticar. Un `SetEnvironmentVariable` con los argumentos al revés guarda la key como *nombre* de la variable, y listar los nombres la vuelca al transcript (mordió el 2026-09-23: dos keys de Anthropic expuestas y revocadas). Mostrar solo nombres que cumplan `^[A-Z_][A-Z0-9_]*$` y **contar** el resto sin imprimirlo; de los valores, solo presencia y largo. Si el conteo de ocultos es mayor que 0: avisar al usuario para que rote esa key y ofrecer borrar la variable por patrón, sin volver a imprimirla.
...
- **Git NO preserva mtimes: después de un pull, la fecha de archivo dice «ahora» y le gana a trabajo local genuinamente más nuevo.** Un checkout estampa la hora actual en todo lo que toca, así que `LastWriteTime` no mide cuándo se editó el CONTENIDO — mide cuándo git lo escribió en disco. Cualquier reconciliación «gana el más nuevo» basada en mtime resuelve al revés justo cuando importa: el archivo recién bajado siempre parece más reciente. Detectado el 2026-09-22 escribiendo `start.ps1` de `claude-continuity`, antes de correrlo — la primera versión habría pisado con la copia del repo la memoria que se acababa de fusionar a mano. **El reloj honesto del lado del repo es la fecha de COMMIT del archivo** (`git log -1 --format=%cI -- <ruta>`), que es independiente de la máquina. Y como datar cuesta, **comparar hashes primero** y fechar solo los que difieren. Corolario general: antes de comparar dos archivos por fecha, preguntar **quién escribió ese timestamp** — si fue una herramienta de sincronización y no un editor, el dato no significa lo que parece.
...
- **Un `grep` por línea sobre registros de varias líneas ve solo la primera y da un "no está" falso.** El 2026-09-23, `grep '^ENTREVISTADOR'` sobre transcripciones de HireSignal mostró turnos sin ninguna pregunta: el entrevistador escribía en dos párrafos y la pregunta vivía en la segunda línea. Casi se reporta que un modelo "no pregunta". Para registros de varias líneas, extraer el bloque entero hasta el siguiente marcador (`awk '/^INICIO/{on=1} /^FIN/{on=0} on'`) antes de juzgar su contenido. **Cuándo sospechar:** si el formato no garantiza una línea por registro (transcripciones, logs con stack traces, markdown), asumir varias. **Comprobación:** leer entero un registro de la muestra antes de contar. Es la 4.ª pregunta de arriba: ¿estoy midiendo la región relevante?
- **Un test de escape que solo afirma "la versión cruda NO aparece" da verde aunque el input nunca haya llegado.** Afirmar también lo positivo: la versión **escapada** tiene que estar. Si no aparece ni cruda ni escapada, el valor no llegó a la página y el test no probó nada. El 2026-09-22 un test de XSS de HireSignal (`<script>` en un campo de vista previa) dio FAIL con las dos en falso: el arnés rellenaba `$_GET` y la página lee `$_REQUEST`. Con solo la aserción negativa habría salido verde sin haber probado el escape.

- **Gamma ignora sus propios parametros de generacion, y el modo `preserve` NO preserva: reescribe el texto e INVENTA estadisticas con cita de fuente.** El 2026-09-07, generando el deck de Twilight Medical con `textMode: "preserve"` + `cardSplit: "inputTextBreaks"` (elegidos justamente para que las cifras medidas no derivaran), la salida traia: tema `blues` cuando se pidio `slate`; seis imagenes AI cuando se pidio `imageOptions: {source: "themeAccent"}`; el texto reescrito ("your real treatment types" donde el original decia "modalities" — error de dominio, esa empresa repara equipos, no trata pacientes); y sobre todo **un dato que nadie escribio**: "Rich results earn 58 clicks per 100 searches versus 41 — a 41% CTR advantage", atribuido a "Milestone Internet, Organic Click Curve Research (2020), based on 2.7 billion Google impressions". La aritmetica cerraba (58/41 = +41%) y la fuente es una firma real, que es justo lo que la hacia creible. **Ningun parametro de Gamma es vinculante — son sugerencias.** Procedimiento: (1) `read_gamma` SIEMPRE despues de generar y diffear contra el `inputText` enviado, antes de dar el link por bueno; (2) en el prompt de cualquier pase de edicion, cerrar con una prohibicion explicita de agregar cifras, porcentajes, benchmarks o citas — un pase de edicion es otra oportunidad de inventar; (3) las cifras dentro de un `infographic` generado viven en pixeles y NO se pueden verificar leyendo el deck: revisarlas a ojo o borrar el bloque. Esto es un generador que **sobre**-reporta, como el medidor hardcodeado de mas abajo, no un buscador que sub-reporta.
- **Un `[System.Environment]::SetEnvironmentVariable(..., "User")` recién corrido en la MISMA sesión no lo ve ningún proceso ya en marcha — ni la propia sesión de Claude Code.** El registro queda escrito bien (se confirma releyéndolo con `GetEnvironmentVariable(...,"User")`), pero eso no prueba que la variable esté disponible para el siguiente comando dentro de la misma sesión: solo un proceso genuinamente nuevo, abierto después de reiniciar la sesión/terminal, la hereda. Es el mismo patrón que el PATH tras un `winget install` ("restart your shell to use the new value"), pero aplica a cualquier variable de usuario. Mordió el 2026-09-14 instalando WhisperX en el portátil: configuré `HF_HOME`/`TORCH_HOME` apuntando a `D:\Transcriptor\`, y la corrida de WhisperX en esa misma sesión igual descargó ~3,3GB a `C:\Users\andre\.cache\` (la ruta default), porque el proceso heredó el entorno viejo. **Si una variable recién configurada se necesita en la misma sesión donde se configuró, pasarla inline en esa llamada** (`$env:VAR = "valor"` antes del comando) — no confiar en que ya quedó persistida hasta la próxima sesión.

- **WebFetch convierte HTML a markdown y no puede ver contenido dentro de `<script>` — reporta "sin GA4/sin schema markup" en sitios que sí los tienen.** El 2026-09-14, auditando 2 leads reales para un pitch de GEO, WebFetch declaró "no schema, no GA4" en los dos sitios; correr el script determinístico real de producción (`the-freelancer/freelancer/ai-visibility-precheck.js`, que hace `fetch()` crudo y regex sobre `<script type="application/ld+json">`) mostró que ambos tenían schema JSON-LD válido — WebFetch simplemente nunca lo vio porque el markdown descarta las etiquetas `<script>`. De paso destapó un bug real en ese mismo script (`extractJsonLd` no leía `@type` dentro de `@graph`, el patrón que usan Yoast/RankMath) — corregido y verificado contra los dos sitios. **Para cualquier hallazgo sobre GA4, schema.org, o cualquier otro contenido que viva dentro de un `<script>`, usar `curl`/fetch crudo o un script determinístico — nunca WebFetch.**
- **Un nombre de decisor sacado de un registro corporativo (D&B, ZoomInfo, filing estatal) puede estar desactualizado — verificar el estado laboral actual en LinkedIn antes de nombrarlo en un artefacto de salida.** El 2026-09-14 dos "officers" de una LLC salieron de un filing de D&B; uno de los dos aparecía en LinkedIn como "Former — Principal" — ya no estaba en la empresa. Se corrigió antes de que el nombre llegara a un email real, pero ya había quedado escrito en un reporte de leads y en una auditoría publicada.
- **Python en Windows decodifica y codifica la tubería como cp1252 — fabrica mojibake donde los bytes están bien.** Por stdin, un `—` UTF-8 llega como `â€”`; por stdout sale como `�`. Mordió dos veces en HireSignal: el 2026-09-17 se construyó y desplegó una "reparación de títulos garbled" (v1.2.1) que devolvió `0 fixed` — un endpoint de diagnóstico mostró `e2 80 94` correcto en producción y se retiró como falso positivo (v1.2.3); el mismo día el título de KT-004 volvió a salir con `�` al imprimirlo. **Antes de declarar texto dañado, mirar los bytes crudos** (`curl | od -c`, o `sys.stdin.buffer`) o correr con `PYTHONIOENCODING=utf-8`.
- **`escapeshellarg()` de PHP en Windows cambia `%` por espacio.** Un argumento `%40recruiting` llegó roto al subproceso y dio **15 FAIL falsos** en un test de render (2026-09-17, HireSignal). Para pasar datos a un subproceso PHP en Windows, escribirlos a un archivo JSON temporal y pasar la ruta.
- **En emulación móvil del navegador, `scrollWidth > innerWidth` dice "sin scroll horizontal" aunque lo haya**: el navegador móvil ensancha el viewport de layout para que el contenido quepa, así que `innerWidth` sube junto con el desborde. Medido el 2026-09-17 (Edge headless por CDP, 390 px): Pool e Integrations daban `hScroll:false` con un documento de **576 px**. **Comparar el ancho del documento contra el ancho del dispositivo** (390), no contra `innerWidth`.
- **Un `.focus()` por script no activa `:focus-visible`** — un chequeo de "¿se ve la descripción con el teclado?" dio `hidden` falso. Con eventos de tecla Tab reales (`Input.dispatchKeyEvent` en CDP) la misma descripción dio `visible` (2026-09-17). Para probar accesibilidad de teclado, simular la tecla, no llamar `.focus()`.
- **Gmail agrupa un correo `Re: <asunto>` en la conversación del correo original y muestra el título SIN `Re:`** — parece que el prefijo no se aplicó. *Consistente con lo observado, no reproducido aislado* (2026-09-17: con el mismo rol de muestra el follow-up de prueba "no tenía Re:"; con otro rol, asunto nuevo, sí lo mostró). Para verificar un asunto enviado: **⋮ → Mostrar original** y leer la línea `Subject:`.
